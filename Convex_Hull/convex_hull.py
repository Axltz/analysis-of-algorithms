...
def main():
    # TODO: cambia la ruta al CSV que te indicó el profesor (listo)
    ruta = "puntos.csv"
...
def cargar_puntos():
    ...
    if ruta:
        puntos_globales = leer_puntos_csv(ruta)
        lbl_cargados.config(text=f"Puntos cargados: {len(puntos_globales)}")

def calcular_hull():
    if not puntos_globales:
        messagebox.showwarning("Error", "Carga puntos primero")
        return
    hull = convex_hull(puntos_globales)
    dibujar(puntos_globales, hull)

ventana = tk.Tk()
ventana.title("Convex Hull - Jarvis March")
ventana.geometry("220x320")

tk.Button(ventana, text="Cargar CSV", command=cargar_puntos, width=15).place(x=30, y=60)

tk.Button(ventana, text="Calcular Hull", command=calcular_hull, width=15, height=2).place(x=30, y=110)

lbl_cargados = tk.Label(ventana, text="Puntos cargados: 0", font=("Arial", 10, "bold"))
lbl_cargados.place(x=30, y=170)
